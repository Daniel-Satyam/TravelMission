sap.ui.define(
  ...
  function (
    ...
    return BaseController.extend("ui5appuat.controller.ApproveClaim", {
      ...
      onInit: function (evt) {
        ...
        oRouter
          .getRoute("approveclaim")
          .attachMatched(this._onRouteMatched, this);
      },

      _onRouteMatched: function (oEvent) {
        var oArgs = oEvent.getParameter("arguments");
        this.claimId = oArgs.claim;
        this.initialize();
      },
      ...
      prepareMissionForUpdate: async function () {
        ...
        aMembers.forEach((oMember) => {
          let oMemberRequest = {
            ...
            reservedBudget: oMember.reservedBudget,
            itinerary: [],
          };
      ...
      animateSaveButton: function () {
        const oButton = this.byId("idUpdateClaimButton");
        if (oButton) {
      ...
      initialize: async function () {
        ...
        this.setModel(oViewModel, "approveClaimModel");
      ...
      initiateClaimAttachment: function () {
        ...
        return new Promise(async function (resolve, reject) {
          ...
          that
            .getView()
            .setModel(claimAttachmentsModel, "claimAttachmentsModel");
        ...
        return new Promise(async function (resolve, reject) {
          ...
          jQuery.ajax({
            ...
            success: async function (data, textStatus, jqXHR) {
              ...
              var claimInfo = claimData.d.results[0];

              claimInfo.cust_startDate = formatter.formatDateStr(
                claimInfo.cust_startDate,
              );
              claimInfo.cust_EndDate = formatter.formatDateStr(
                claimInfo.cust_EndDate,
              );
              ...
              var missionId = claimInfo.cust_MissionID;
              var employeeId = claimInfo.cust_EmployeeID;
      ...
      approveClaim: async function (oEvent) {
        const that = this;
        const envInfo = await that.getEnvInfo();
        const decryptedDataParsed = await that.getTravelStorage();
        ...
        const oMe = _.find(aMembers, (m) => {
          if (
            m.employeeID === decryptedDataParsed.keyVault.user.id ||
            m.employeeID === decryptedDataParsed.keyVault.masterUser.id
          ) {
            return true;
          }
        });
        ...
        var aInfoCalculate = this.getView()
          ...
          .getData().info;
        var claimInfo = this.getView()
          .getModel("claimInfoModel")
          .getData().info;
        ...
        jQuery.ajax({
          ...
          success: async function (data, textStatus, jqXHR) {
            var sectorDecryptedData = await that.getDecryptedData(data);

            var sectorData = JSON.parse(sectorDecryptedData);

            sectorUtilizedBudget = sectorData.d.results[0].cust_Utilized_Budget;
