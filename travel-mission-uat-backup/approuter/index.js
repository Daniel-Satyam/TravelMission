...
const _fetchSectors = async function (cookies) {
  try {
    const tokenUrl =
      cookies.SF.URL +
      "cust_SectorBudget?$format=json&$select=externalCode,externalName_localized,externalName_ar_SA,cust_Utilized_Budget,cust_Parked_Amount,cust_Budget,cust_Available_budget,cust_Delegate_Approver_for_Missions,cust_Head_of_Sector,cust_Delegate_Dynamic_group,cust_S4_Sector,cust_S4_SubSector,cust_Visible";
    var auth = "Basic " + cookies.SF.basicAuth;
    const config = {
      headers: {
        Authorization: auth,
...
const _fetchMissionBudgetInfo = async function (cookies) {
  try {
    const tokenUrl =
      cookies.SF.URL +
      "cust_SectorBudget?$format=json&$select=externalCode,externalName_localized,externalName_ar_SA,cust_Utilized_Budget,cust_Parked_Amount,cust_Budget,cust_Available_budget,cust_S4_Sector,cust_S4_SubSector,cust_Visible";
    var auth = "Basic " + cookies.SF.basicAuth;
    const config = {
      headers: {
        Authorization: auth,
...
const _updateItineraryBatch = async function (body, cookies) {
  try {
    ...
    const selectQuery =
      ...
      "cust_Members/cust_Employee_Total_Perdiem,cust_Members/cust_Reserved_Budget,cust_Members/cust_Employee_Total_Expense," +
    ...
    if (batchResult[0] && batchResult[1]) {
      ...
      missionData.cust_Members.results.forEach((oMember) => {
        ...
        if (oMemberFound) {
          let memberUpdateRequest = {
            ...
            cust_Reserved_Budget: oMemberFound.reservedBudget,
      ...
      if (postBatchResult && postBatchResult[0] && postBatchResult[0].error) {
        console.log(
          "Error during updating itinerary:",
          postBatchResult[0].error.value,
        );
        throw new Error(
          `Error during updating itinerary: ${postBatchResult[0].error.value}`,
        );
      }
  ...
  try {
    ...
    if (
      ...
      const missionSelectQuery =
        ...
        "cust_Members/cust_Employee_Total_Perdiem,cust_Members/cust_Reserved_Budget,cust_Members/cust_Employee_Total_Expense,cust_Members/cust_Claimed," +
        "cust_Members/cust_itinerary_details_child/cust_Mission_effectiveStartDate," +
        "cust_Members/cust_itinerary_details_child/cust_Mission_externalCode,cust_Members/cust_itinerary_details_child/cust_Members_externalCode," +
        "cust_Members/cust_itinerary_details_child/cust_Mission_transactionSequence,cust_Members/cust_itinerary_details_child/externalCode," +
        "cust_Members/cust_itinerary_details_child/cust_start_date,cust_Members/cust_itinerary_details_child/cust_end_date," +
      ...
      if (postBatchResult && postBatchResult[0] && postBatchResult[0].error) {
        console.log(
          "Error during claiming mission:",
          postBatchResult[0].error.value,
        );
        throw new Error(
          `Error during claiming mission: ${postBatchResult[0].error.value}`,
        );
      }
          ...
          if (missionFetchResponse && missionFetchResponse.data) {
            ...
            if (missionUpdateResponse && missionUpdateResponse.data) {
              ...
              if (sectorFetchResponse && sectorFetchResponse.data) {
                ...
                const postBatchResponse = await axios.post(
                  batchURL,
                  postBatchBody,
                  {
                    headers: {
...
const _fetchAdvances = async function (decryptedData, cookies) {
  try {
    ...
    if (sFilterOptions) {
      var getAdvanceUrl =
        cookies.SF.URL +
        `cust_TravelAdvance?$format=json&$expand=cust_EmployeeIDNav&$select=externalCode,mdfSystemEffectiveStartDate,cust_MissionID,cust_EmployeeID,cust_StartDate,cust_EndDate,cust_location,cust_Status,cust_EmployeeIDNav/defaultFullName&$filter=${sFilterOptions}`;
  ...
  try {
    ...
    if (missionFetchResponse && missionFetchResponse.data) {
      ...
      const sectorFetchUrl =
        ...
        "cust_SectorBudget?$format=json&$filter=externalCode eq " +
        missionUpdateRequest.cust_Sector +
        "&$select=externalCode,cust_Delegate_Approver_for_Missions,cust_Head_of_Sector,cust_Available_budget,cust_Parked_Amount,cust_Utilized_Budget,effectiveStartDate,cust_S4_Sector,cust_S4_SubSector,cust_Visible";
      const sectorFetchConfig = {
        headers: {
          Authorization: auth,
        },
...
const _approveRejectCancel_v1 = async function (decryptedData, cookies) {
  try {
    var auth = "Basic " + cookies.SF.basicAuth;
...
const _approveRejectCancel = async function (decryptedData, cookies) {
  try {
    ...
    if (batchResult[0] && batchResult[1] && batchResult[2]) {
      const sectorFetchResponse = _.cloneDeep(batchResult[0]);
      const missionUpdateRequest = _.cloneDeep(batchResult[1].d.results[0]);
      const getApproveGroupResponse = _.cloneDeep(batchResult[2].d.results[0]);
      const approveGroup = getApproveGroupResponse.cust_Value;

      const oSubSector = _.find(sectorFetchResponse.d.results, [
        "externalCode",
        missionUpdateRequest.cust_Sector,
      ]);

      const oMainSector = oSubSector
        ? _.find(sectorFetchResponse.d.results, {
            cust_S4_Sector: oSubSector.cust_S4_Sector,
            cust_S4_SubSector: oSubSector.cust_S4_Sector,
          })
        : null;
    ...
    if (batchResult[0] && batchResult[1]) {
      ...
      body.members.forEach((oMember) => {
        let memberUpdateRequest = {
          ...
          cust_Reserved_Budget: _convertFloat(oMember.reservedBudget),
          ...
          for (var j = 0; j < body.members.length; j++) {
            ...
            if (attachmentId == null) {
              memberUpsertReq = {
                ...
                cust_Reserved_Budget: body.members[j].reservedBudget,
...
const _constructS4DocumentDetailsFromMissionId = async function (
  ...
  try {
    ...
    const missionFilterQuery = `externalCode eq '${missionId}'`;
    const missionSelectQuery =
      "externalCode,cust_Mission_Start_Date,cust_Mission_End_Date,cust_No_Of_Days," +
      ...
      "cust_Members/cust_Employee_Total_Ticket,cust_Members/cust_Employee_Total_Perdiem,cust_Members/cust_Reserved_Budget,cust_Members/cust_Employee_Total_Expense," +
    ...
    if (
      ...
      missionFetchResponse.data.d.results
    ) {
      const oMission = missionFetchResponse.data.d.results[0];
      const oPayload = {
        __metadata: oMission.__metadata,
        missionId: oMission.externalCode,
        s4DocumentNumber: oMission.cust_S4_Document_Number,
        members: [],
      };

      let employeeFilterQuery = "";

      oMission.cust_Members &&
        oMission.cust_Members.results &&
        oMission.cust_Members.results.forEach((m, i) => {
          ...
          employeeFilterQuery =
            employeeFilterQuery === ""
              ? `userId in '${m.cust_Employee_ID}'`
              : `${employeeFilterQuery},'${m.cust_Employee_ID}'`;
        });

      if (employeeFilterQuery === "") {
        throw new Error("Mission member list empty");
      }
      ...
      if (
        ...
        oPayload.members.forEach((m, i) => {
          ...
          if (!oEmp) {
            ...
            oPayload.members[i].costCenter = oEmp.costCenter;
          }
        });

        if (bMissing) {
          throw new Error("Not all employee data fetched");
        }

        return oPayload;
      } else {
        throw new Error("Employee list fetch error");
      }
    } else {
      throw new Error("Mission fetch error");
    }
  } catch (e) {
    throw new CustomHttpError(
      500,
  ...
  try {
    ...
    if (missionRequest.info.missionId && referenceGuid === null) {
      ...
      oMissionRequest.members.forEach((m) => {
        if (!m.costCenter) {
          //--member does not have a cost center
          employeeFilterQuery =
            employeeFilterQuery === ""
              ? `userId in '${m.userID}'`
...
const _getMastersBatch = async function (body, cookies) {
  ...
  try {
    ...
    const sectorFetchUrl =
      "cust_SectorBudget?$format=json&$select=externalCode,externalName_localized,externalName_ar_SA,cust_Utilized_Budget,cust_Parked_Amount,cust_Budget,cust_Available_budget,cust_Delegate_Approver_for_Missions,cust_Head_of_Sector,cust_Delegate_Dynamic_group,cust_S4_Sector,cust_S4_SubSector,cust_Visible,cust_Decree_Type";
      ...
      reportResponse.data.d.results.forEach((m0) => {
        ...
        if (m0.cust_Members && m0.cust_Members.results) {
          m0.cust_Members.results.forEach((m1, i) => {
            ...
            // resultRow[`reservedBudget`] = _formatCurrency(
            //   m1.cust_Reserved_Budget,
            // );
...
ar.beforeRequestHandler.use("/fetchSectors", async (req, res, next) => {
  try {
    const reqCookies = await _fetchCookies(req);
    let mastersObj = {
      sectors: [],
    ...
    if (sectors && sectors.d && sectors.d.results) {
      mastersObj.sectors = sectors.d.results;
    }
...
ar.beforeRequestHandler.use("/getMissionById", async (req, res, next) => {
  try {
    ...
    try {
      missionInfo.auditLogs.sort((a, b) => {
...
ar.beforeRequestHandler.use("/createS4Document", async (req, res, next) => {
  try {
    const reqCookies = await _fetchCookies(req);
...
ar.beforeRequestHandler.use("/deleteS4Document", async (req, res, next) => {
  try {
    const reqCookies = await _fetchCookies(req);
...
// ar.beforeRequestHandler.use("/updateS4Document", async (req, res, next) => {
//   try {
//     const reqCookies = await _fetchCookies(req);
