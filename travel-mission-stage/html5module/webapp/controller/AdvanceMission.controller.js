sap.ui.define([
	...
	return BaseController.extend("ui5appstage.controller.AdvanceMission", {
		...
		onInit: function (evt) {
			...
			oRouter.getRoute("advancemission").attachMatched(this._onRouteMatched, this);
		},
		...
		preInitializeModel: function() {
			...
			return new Promise(async function(resolve, reject) {
				...
				if(decryptedDataParsed.keyVault.permission.mission.isAdvance  != true){
					reject(false);
				} else {
					var screenModelData = {
						pageError: false,
		...
		advanceMission: async function() {
			...
			var mModelDataCalculate   = this.getModel("membersModel").getData().members; 
			
			var aInfoCalculate = this.getModel("missionInfoModel").getData().info;
			
			var decree = this.getModel('missionAttachmentsModel').getData().attachments;
			...
			for(var i=0;i<mModelDataCalculate.length;i++) {
				...
				for(var j=0;j<itineraryData.length;j++) {
					...
					itineraryStartDate.setTime(itineraryStartDate.getTime() + hoursToAdd);
					...
					if(!isNaN(itineraryData[j].perDiemPerCity)) { 
						...
						if(itineraryData[j].perDiemPerCity.indexOf(",") > -1) {
							itineraryPerDiem = parseFloat(itineraryData[j].perDiemPerCity.replace(/\,/g,''));
						} else {
							itineraryPerDiem = parseFloat(itineraryData[j].perDiemPerCity);
						}
					}
					memberPerDiemPerCityCalculate = memberPerDiemPerCityCalculate + itineraryPerDiem;
				}
