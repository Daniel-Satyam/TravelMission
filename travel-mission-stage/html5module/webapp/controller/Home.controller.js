sap.ui.define(
  ...
  function (
    ...
    return BaseController.extend("ui5appstage.controller.Home", {
      formatter: formatter,
        ...
        return new Promise(function (resolve, reject) {
          jQuery.ajax({
            ...
            success: async function (data, textStatus, jqXHR) {
             ...
             // decryptedDataParsed.keyVault.user.email = "Shaikha.Almenhali@mofa.gov.ae"; //--Stage
             // decryptedDataParsed.keyVault.user.email = "shamma.almarzooqi@mofa.gov.ae"; //--Stage
             // decryptedDataParsed.keyVault.user.email = "ali_almazrouei@mofa.gov.ae"; //--HOS
             // decryptedDataParsed.keyVault.user.email = "y_alshehhi@mofa.gov.ae"; //--GS Group
             // decryptedDataParsed.keyVault.user.email = "hashim.alhashmi@mofa.gov.ae"; //--Stage
             // decryptedDataParsed.keyVault.user.email = "nuha.alhemeiri@mofa.gov.ae"; //--Stage
             // decryptedDataParsed.keyVault.user.email = "Mouza.Alnaqbi@mofa.gov.ae"; //--Stage
             //decryptedDataParsed.keyVault.user.email = "h.musleh@mofa.gov.ae"; //--Stage
             //decryptedDataParsed.keyVault.user.email = "s.aljarwan@mofa.gov.ae"; //--Stage
             // decryptedDataParsed.keyVault.user.email = "Omran.Sharaf@mofa.gov.ae"; //--Stage
             //decryptedDataParsed.keyVault.user.email = "Abdulla.Aldahmani@mofa.gov.ae"; //--Stage
            
             if(sEmail){
              decryptedDataParsed.keyVault.user.email = sEmail;
             }else{
              decryptedDataParsed.keyVault.user.email =  data.email;
             }
             const getEncryptedData = await that.getEncryptedData(
               decryptedDataParsed
             );
      ...
      selectFromPresets: async function (oEvent) {
        ...
        if (!this._oPresetMenuFragment) {
          this._oPresetMenuFragment = await Fragment.load({
            id: oView.getId(),
            name: "ui5appstage.view.fragments.PresetDateSelection",
            controller: this,
          });
      ...
      tableFilter: function (type, oEvent) {
        if (type == "mission") {
          ...
          if (sQuery && sQuery.length > 0) {
            for (var i = 0; i < originalMissionsModelData.length; i++) {
              if ( 
                ...
                ( ( 
                    originalMissionsModelData[i].destination.localeLabel.toLowerCase().indexOf(sQuery.toLowerCase()) > -1  ||
                    originalMissionsModelData[i].destination.labelAr.toLowerCase().indexOf(sQuery.toLowerCase()) > -1 
                  ) ) 
              ){
          ...
          const originalClaimsModelData = this.getView()
            ...
            .getData().claims;
          const aDestination = this.getModel("destinationsModel").getProperty("/destinations");
          const sQuery = oEvent.getSource().getValue();
          if (sQuery && sQuery.length > 0) {
            for (var i = 0; i < originalClaimsModelData.length; i++) {
              const oLocation = _.find(aDestination, ["externalCode", originalClaimsModelData[i].cust_Location]);
              const sMissionId = originalClaimsModelData[i].cust_MissionID;
              const sEmployeeName = ( originalClaimsModelData[i].cust_EmployeeIDNav && originalClaimsModelData[i].cust_EmployeeIDNav.defaultFullName ) || null;
              ...
              if ( 
                    ...
                    ( oLocation && ( oLocation.localeLabel.indexOf(sQuery) > -1  || oLocation.localeARLabel.indexOf(sQuery) > -1 ) ) 
              ) {
          ...
          if (sQuery && sQuery.length > 0) {
            for (var i = 0; i < originalAdvancesModelData.length; i++) {
              const oLocation = _.find(aDestination, ["externalCode", originalAdvancesModelData[i].cust_location]);
              const sMissionId = originalAdvancesModelData[i].cust_MissionID;
              const sEmployeeName = ( originalAdvancesModelData[i].cust_EmployeeIDNav && originalAdvancesModelData[i].cust_EmployeeIDNav.defaultFullName ) || null;
      ...
      callClaimAdvanceFilter: async function () {
        if (!this._oClaimAdvanceFilterDialog) {
          this._oClaimAdvanceFilterDialog = await Fragment.load({
            ...
            name: "ui5appstage.view.fragments.ClaimAdvanceFilterDialog",
            controller: this,
          });
      ...
      onSelectDelegator: async function (oEvent) {
        ...
        if (!this._oDelegatorSelectDialog) {
          this._oDelegatorSelectDialog = await Fragment.load({
            id: this.getView().getId(),
            name: "ui5appstage.view.fragments.SelectDelegator",
            controller: this,
          });
