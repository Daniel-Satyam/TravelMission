sap.ui.define(
  ...
  function (
    ...
    return Controller.extend("ui5appstage.controller.BaseController", {
       ...
       * @private
       * @returns {ui5appstage.ui.BusyDialog} the router for this component
       */
      getBusyFragment: async function () {
        if (!this.oBusyDialog) {
          this.oBusyDialog = await Fragment.load({
            id: this.getView().getId(),
            name: "ui5appstage.view.fragments.GenericBusyDialog",
            controller: this,
          });
      ...
      onOverflowMenuOpen: function () {
        ...
        if (!this._oMenuFragment) {
          this._oMenuFragment = Fragment.load({
            id: oView.getId(),
            name: "ui5appstage.view.fragments.HeaderOverflowMenuPopover",
            controller: this,
          }).then(function (oMenu) {
      ...
      getDecryptedData: function (data) {
        ...
        return new Promise(async function (resolve, reject) {
          ...
          const contentBytes = forge.util.hexToBytes(data);
          decipher.update(forge.util.createBuffer(contentBytes));
          decipher.finish();
          //resolve(decipher.output.toString());
          const decryptedUtf8 = forge.util.decodeUtf8(
      ...
      getEncryptedData: async function (data) {
        ...
        return new Promise(async function (resolve, reject) {
          ...
          const contentUtf8 = forge.util.encodeUtf8(JSON.stringify(data));
          cipher.update(forge.util.createBuffer(contentUtf8));
          cipher.finish();
      ...
      onOpenErrorPane: async function () {
        ...
        if (!this._oErrorPane) {
          this._oErrorPane = await Fragment.load({
            id: oView.getId(),
            name: "ui5appstage.view.fragments.ErrorPane",
            controller: this,
          });
                  ...
                  if (c && c.d && c.d.results && c.d.results.length > 0) {
                    c.d.results.forEach((r) => {
                      if (r["cust_Mission_Start_Date"]) {
                        ...
                        aErrors.push(oError);
                      } else if (r["cust_Employee_ID"]) {
                        r.cust_itinerary_details_child.results.forEach((p) => {
                          //--Duplicate members
