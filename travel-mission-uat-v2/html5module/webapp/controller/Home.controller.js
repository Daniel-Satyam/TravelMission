sap.ui.define(
  ...
  function (
    ...
    return BaseController.extend("ui5appuat.controller.Home", {
      formatter: formatter,
      ...
      // createS4Document: async function () {
      //   const url = "/createS4Document";
      //   const envInfo = await this.getEnvInfo();
      ...
      //   });
      // },
      // deleteS4Document: async function () {
      //   const url = "/deleteS4Document";
      //   const envInfo = await this.getEnvInfo();
      ...
      // updateS4Document: async function () {
      //   const url = "/updateS4Document";
      //   const envInfo = await this.getEnvInfo();
      ...
      initializeModel: function () {
        ...
        return new Promise(async function (resolve, reject) {
          ...
          oScreenModel.setProperty("/info", oScreenModelData);
           ...
           //--Register for sort
          that._registerForP13n("idHomeClaimsTable");
          that._registerForP13n("idHomeAdvancesTable");
          that._registerForP13n("idHomeMissionsTable");
      ...
      selectFromPresets: async function (oEvent) {
        ...
        if (!this._oPresetMenuFragment) {
          this._oPresetMenuFragment = await Fragment.load({
            id: oView.getId(),
            name: "ui5appuat.view.fragments.PresetDateSelection",
            controller: this,
          });
      ...
      tableFilter: function (type, oEvent) {
        if (type == "mission") {
          ...
          const originalMissionsModelData = this.getView()
      ...
      callClaimAdvanceFilter: async function () {
        if (!this._oClaimAdvanceFilterDialog) {
          this._oClaimAdvanceFilterDialog = await Fragment.load({
            ...
            name: "ui5appuat.view.fragments.ClaimAdvanceFilterDialog",
            controller: this,
          });
      ...
      onSelectDelegator: async function (oEvent) {
        ...
        if (!this._oDelegatorSelectDialog) {
          this._oDelegatorSelectDialog = await Fragment.load({
            id: this.getView().getId(),
            name: "ui5appuat.view.fragments.SelectDelegator",
            controller: this,
          });
      ...
      //--P13n methods
      _getP13nSource: function (sId) {
        let sSource;
        switch (sId) {
          case "idHomeClaimsTable":
            sSource = "claim";
            break;
          case "idHomeAdvancesTable":
            sSource = "advance";
            break;
          case "idHomeMissionsTable":
            sSource = "mission";
            break;
          default:
            return null;
      ...
      _registerForP13n: function (sId) {
        const sSource = this._getP13nSource(sId);
        const sModelName = `${sSource}P13nModel`;
        ...
        oTable.getColumns().forEach((oColumn) => {
          ...
          const sLabel = oColumn.data("p13nLabel");
          const sPath = oColumn.data("p13nPath");
          if (sPath) {
            aP13nConfig.push({
              key: sKey,
              label: sLabel,
        ...
        const oP13nModel = new JSONModel({
          elementId: sId,
          p13n: aP13nConfig,
          sorters: [],
          sortables: [],
        ...
        this.setModel(oP13nModel, sModelName);
      },
      afterCloseP13nDialog: async function () {
        this._oP13nDialog.destroy();
        this._oP13nDialog = null;
      },
      openP13nDialog: async function (oEvent) {
        const sId = oEvent.getSource().data("sourceTable");
        const oTable = this.byId(sId);
        ...
        const sSource = this._getP13nSource(sId);
        const sModelName = `${sSource}P13nModel`;
        const oP13nModel = this.getModel(sModelName);

        if (!oP13nModel) {
          return;
        }

        const aP13nConfig = oP13nModel.getProperty("/p13n");
        const aSortable = [];
        let aSorters = [];
        oTable.getColumns().forEach((oCol) => {
          if (oCol.data("p13nKey") !== null) {
            const oP13n = _.find(aP13nConfig, ["key", oCol.data("p13nKey")]);
            if (oP13n.sorted === true) {
              aSorters.push({
                ...oP13n,
                deletable: true,
              });
            }
            aSortable.push(_.clone(oP13n));
          }
        });

        aSorters = _.sortBy(_.clone(aSorters), ["sortIndex"]);
        ...
        aSorters.push({
          ...
          sorted: false,
          sortOrder: "None",
          sortIndex: aSorters.length + 1,
          deletable: false,
        });

        oP13nModel.setProperty("/sorters", aSorters);
        oP13nModel.setProperty("/sortables", aSortable);
        
        const oView = this.getView();

        if (!this._oP13nDialog) {
          this._oP13nDialog = await Fragment.load({
            id: oView.getId(),
            name: "ui5appuat.view.fragments.TableP13nDialog",
            controller: this,
          });

          oView.addDependent(this._oP13nDialog);
        }

        this._oP13nDialog.setModel(oP13nModel);
        this._oP13nDialog.open();
      },
      onSortableFieldSelected: function (oEvent) {
        const oComboBox = oEvent.getSource();
        const sKey = oComboBox.getSelectedKey() || null;
        if (!sKey) return;
        if (!this._oP13nDialog) return;

        const oP13nModel = this._oP13nDialog.getModel();
        const aP13n = oP13nModel.getProperty("/p13n");
        const aSorters = oP13nModel.getProperty("/sorters");
        
        if (sKey) {
          aSorters.forEach((oSorter, i) => {
            if (oSorter.key){
              oSorter.deletable = true; 
              if (oSorter.sortOrder === "None"){
                oSorter.sortOrder = "Ascending";
              }
              oSorter.sorted = true;
              const j = _.findIndex(aP13n, ["key", oSorter.key]);
              if (j >= 0) {
                oSorter.label = aP13n[j].label;
                oSorter.path = aP13n[j].path;
              }
            }

            oSorter.sortIndex = i + 1;
          });
        }

        const i = _.findIndex(aSorters, ["key", null]);
      ...
      onSortableFieldRemoved: function (oEvent) {
        if (!this._oP13nDialog) return;

        const oP13nModel = this._oP13nDialog.getModel();
        const oSource = oEvent.getSource();
        const sKey = oSource.data("key");
        const aSorters = oP13nModel.getProperty("/sorters");

        const i = _.findIndex(aSorters, ["key", sKey]);
      ...
      onP13nApply: function(){
        if (!this._oP13nDialog) return;
        const oP13nModel = this._oP13nDialog.getModel();
        const sId = oP13nModel.getProperty("/elementId");
        const aSorters = oP13nModel.getProperty("/sorters");
        const aP13n = oP13nModel.getProperty("/p13n");
        const oTable = this.byId(sId);
        ...
        aSorters.forEach((oSorter)=>{
          if(oSorter.path){
            aSort.push(new Sorter(
              oSorter.path,
              oSorter.sortOrder === "Descending"
            ));
          }
        });

        aColumns.forEach((oCol)=>{
          const sKey = oCol.data("p13nKey");
          if(sKey){
            const oSorter = _.find(aSorters, ["key", sKey]);
            const oP13n = _.find(aP13n, ["key", sKey]);
            if(oSorter){
              oP13n.sorted = true;
              oP13n.sortOrder = oSorter.sortOrder;
              oP13n.sortIndex = oSorter.sortIndex;
            }else{
              oP13n.sorted = false;
              oP13n.sortOrder = "None";
              oP13n.sortIndex = null;
            }
            oCol.setSortIndicator(oP13n.sortOrder);
          }
        });
        
        oP13nModel.setProperty("/p13n", aP13n);
        
        oTable.getBinding("items").sort(aSort);
        this._oP13nDialog.close();
      },

      onP13nCancel: function(){
        if (!this._oP13nDialog) return;
        this._oP13nDialog.close();
      },

      getP13nSourceTable: function (oInfo, oPayrollGroup) {
        if (oInfo.claimsTab === true) {
          return "idHomeClaimsTable";
        } else if (oInfo.advanceTab === true) {
          return "idHomeAdvancesTable";
        } else if (
          oInfo.list === true ||
          (oInfo.payrollGroup === true && oPayrollGroup.showMissions === true)
        ) {
          return "idHomeMissionsTable";
        }
      },
      //--P13n methods
    });
